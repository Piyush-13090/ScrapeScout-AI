...
export default function SavedPage() {
  ...
  async function handleUpdate(id: string, field: string, value: string) {
    ...
    try {
      ...
      console.error(err);
      // Revert on failure (simple reload for MVP)
      fetchSaved();
    }
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-slate-400 bg-white/5 border border-white/10 px-4 py-2 rounded-lg">
            Total Saved:{' '}
            <span className="text-white font-bold">{savedOpps.length}</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {savedOpps.map((saved) => {
              ...
              if (!opp) return null; // shouldn't happen with our join
                ...
                <div key={saved.id} className="group flex flex-col md:flex-row gap-6 p-5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors">
                  ...
                  <div className="shrink-0 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    <div className="flex flex-col gap-1 w-full sm:w-auto">
                      ...
                      >
                        {Object.keys(STATUS_COLORS).map((status) => (
                          <option key={status} value={status} className="bg-slate-900 text-slate-300">{status}</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="flex items-center self-end sm:self-center pt-5 sm:pt-0 gap-2">
                      ...
                      <button onClick={() => handleRemove(saved.id)} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg transition-colors" title="Remove">
